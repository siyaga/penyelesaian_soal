...
console.log(
  `Kalimat sebelumnya ${data} sesudah di reverse alphabet ${dataReserse.substring(
    1,
    6
  )}${dataReserse.substring(0, 1)} `
);
...
console.log(
  `diagonal pertama = ${diagonalPertama.reduce((acc, curr) => acc + curr)}`
);
console.log(
  `diagonal kedua = ${diagonalKedua.reduce((acc, curr) => acc + curr)}`
);
const hasil =
  diagonalPertama.reduce((acc, curr) => acc + curr) -
  diagonalKedua.reduce((acc, curr) => acc + curr);
console.log(`${hasil}`);
